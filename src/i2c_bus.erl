...
%% @end
%%-----------------------------------------------------------------------------
-spec enqueue(Bus :: i2c_bus(), Address :: address(), Operations :: operations()) ->
...
%% @end
%%-----------------------------------------------------------------------------
-spec write_bytes(Bus :: i2c_bus(), Address :: address(), Bytes :: binary()) ->
    ok | {error, Reason :: term()}.
...
%%-----------------------------------------------------------------------------
-spec write_bytes(
    Bus :: i2c_bus(), Address :: address(), Register :: register(), Bytes :: binary()
) -> ok | {error, Reason :: term()}.
...
%% @end
%%-----------------------------------------------------------------------------
-spec read_bytes(Bus :: i2c_bus(), Address :: address(), Count :: non_neg_integer()) ->
...
%%-----------------------------------------------------------------------------
-spec read_bytes(
    Bus :: i2c_bus(), Address :: address(), Register :: register(), Count :: non_neg_integer()
) -> {ok, binary()} | {error, Reason :: term()}.
...
handle_call({write_bytes, Address, Register, Bytes}, _From, State) ->
    %?TRACE("Writing bytes ~p to address ~p register ~p i2c ~p", [
    %    Bytes, Address, Register, State#state.i2c
    %]),
...
handle_call({read_bytes, Address, Register, Count}, _From, State) ->
    % ?TRACE("Reading bytes off address ~p register ~p count ~p i2c ~p", [
    %     Address, Register, Count, State#state.i2c
    % ]),
